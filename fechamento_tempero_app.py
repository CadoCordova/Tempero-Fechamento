...
def inject_css():
    st.markdown(
        ...
        .tempero-metric-card {{
            ...
            background-image: linear-gradient(135deg, {PRIMARY_COLOR}, {SECONDARY_COLOR});
            color: #ffffff;
        ...
        /* Títulos das seções dentro de cards */
        .tempero-card-title {{
            font-size: 0.95rem;
            font-weight: 700;
            color: {TEXT_DARK};
...
def formatar_tabela_excel(ws, df, start_row=1):
    ...
    fill_header = PatternFill(start_color="FFE5E7EB", end_color="FFE5E7EB", fill_type="solid")
...
def gerar_caixa_diario(df_consolidado: pd.DataFrame, saldo_inicial: float) -> pd.DataFrame:
    ...
    df["Entradas"] = df["Valor"].where(df["Valor"] > 0, 0.0)
    df["Saídas"] = df["Valor"].where(df["Valor"] < 0, 0.0).abs()
    df_caixa = (
        df.groupby("Data")[["Entradas", "Saídas"]]
        .sum()
        .reset_index()
        .sort_values("Data")
    )
    df_caixa["Resultado"] = df_caixa["Entradas"] - df_caixa["Saídas"]
    df_caixa["Saldo Acumulado"] = saldo_inicial + df_caixa["Resultado"].cumsum()
    return df_caixa
...
with st.sidebar:
    ...
    st.markdown(
        "<p>Envie os extratos do Itaú e PagSeguro referentes ao período atual.</p>",
        unsafe_allow_html=True,
    )
    ...
    saldo_inicial_input_sidebar = st.text_input(
        "Saldo inicial consolidado do período (R$)",
        value="0",
        help="Saldo das contas + dinheiro em caixa no início do período.",
    )
    ...
    if st.session_state.get("auth_ok"):
        ...
        if st.button("Sair", key="logout_button_sidebar"):
...
with tab2:
    ...
    if mensagem_erro:
        ...
        entradas_dinheiro_periodo = df_dinheiro_periodo.loc[
            df_dinheiro_periodo["Tipo"] == "Entrada", "Valor"
        ].sum()
        saidas_dinheiro_periodo = df_dinheiro_periodo.loc[
            df_dinheiro_periodo["Tipo"] == "Saída", "Valor"
        ].sum()
        ...
        with col_c:
            ...
            st.write("Entradas:", formatar_moeda(entradas_dinheiro_periodo))
            st.write(
                "Saídas  :",
        ...
        # Consolidação e caixa diário
        st.markdown(
            '<div class="tempero-section-title">🏁 Caixa diário consolidado</div>',
            unsafe_allow_html=True,
        )
        if not df_caixa_diario.empty:
            df_view_caixa = df_caixa_diario.copy()
            df_view_caixa["Data"] = df_view_caixa["Data"].dt.strftime("%d/%m/%Y")
            df_view_caixa["Entradas"] = df_view_caixa["Entradas"].apply(
                formatar_moeda
            )
            df_view_caixa["Saídas"] = df_view_caixa["Saídas"].apply(formatar_moeda)
            df_view_caixa["Resultado"] = df_view_caixa["Resultado"].apply(
                formatar_moeda
            )
            df_view_caixa["Saldo Acumulado"] = df_view_caixa[
                "Saldo Acumulado"
            ].apply(formatar_moeda)

            st.dataframe(df_view_caixa, use_container_width=True)
        else:
            st.info("Nenhum dado de caixa diário disponível para o período.")
        ...
        df_view["Entradas"] = df_view["Valor"].where(df_view["Valor"] > 0, 0.0)
        df_view["Saídas"] = df_view["Valor"].where(df_view["Valor"] < 0, 0.0)
